...
//!
//! PATCHED: Updated smoothing_flags constants to match current tgstation (2024+).
//! TG removed SMOOTH_CORNERS and shifted SMOOTH_BITMASK to bit 0.
//! See: code/__DEFINES/icon_smoothing.dm in tgstation
...
/// Bitmask smoothing: adjacencies select a pre-baked icon_state by junction value.
/// Current tgstation: SMOOTH_BITMASK = 1 << 0
const SMOOTH_BITMASK: i32 = 1 << 0;
/// Cardinal-only bitmask smoothing (no diagonal junctions).
/// Current tgstation: SMOOTH_BITMASK_CARDINALS = 1 << 1
const SMOOTH_BITMASK_CARDINALS: i32 = 1 << 1;
